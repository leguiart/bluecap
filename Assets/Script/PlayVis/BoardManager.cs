...
LOSE MATCH LINE 3";
        ...
        public void SetupBoard(InteractiveGame game)
        {
            currentGame = game;

            Camera.main.backgroundColor = backgroundColor;

            pieces = new PlayerPiece[(int)game.Genotype[0], (int)game.Genotype[1]];

            GameObject board = new GameObject("Board");

            //! Render the board. By default we start at (0,0) and reposition the camera after
            BoardPiece boardPiece;
            for (int i = 0; i < (int)game.Genotype[0]; i++)
            {
                for (int j = 0; j < (int)game.Genotype[1]; j++)
                {
            ...
            //Set camera centre
            float cx = -0.5f + (float)(int)game.Genotype[0] / 2f;
            float cy = -0.5f + (float)(int)game.Genotype[1] / 2f;
            ...
            float targetRatio = (int)game.Genotype[0] / (int)game.Genotype[1];

            if (screenRatio >= targetRatio)
            {
                Camera.main.orthographicSize = 1.5f + ((int)game.Genotype[1] / 2);
            }
            else
            {
                float differenceInSize = targetRatio / screenRatio;
                Camera.main.orthographicSize = 0.5f + ((int)game.Genotype[0] / 2 * differenceInSize);
            }
        ...
        public void SetEndState()
        {
            ...
            endText.color = c;
            //Set it properly
            if (currentGame.endStatus < BaseGame.END_STATUS_DRAW)
            {
